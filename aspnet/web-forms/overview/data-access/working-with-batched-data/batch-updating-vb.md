...
之后`For Each`循环完成后，`products`包含一个`ProductsRow`GridView 中的每一行的实例。 由于每个`ProductsRow`已添加到实例`products`（而不是已更新），如果我们会盲目地将其传递给`UpdateWithTransaction`方法`ProductsTableAdatper`将尝试将每个记录插入数据库。 相反，我们需要指定这些行的每个已修改了 （未添加）。
