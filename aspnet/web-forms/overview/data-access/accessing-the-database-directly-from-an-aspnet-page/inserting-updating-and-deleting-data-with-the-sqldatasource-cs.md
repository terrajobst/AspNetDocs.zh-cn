...
我们探讨了与对象的数据源中使用乐观并发[实现乐观并发](../editing-inserting-and-deleting-data/implementing-optimistic-concurrency-cs.md)教程。 SqlDataSource 控件还提供开放式并发支持。 如自动生成时在步骤 2 中所述`INSERT`， `UPDATE`，和`DELETE`语句，该向导提供了使用开放式并发选项。 我们将在下一教程中看到的通过 SqlDataSource 使用乐观并发修改`WHERE`中的子句`UPDATE`和`DELETE`语句，以确保其他列的值还 t 自上次数据更改在页面上显示。
