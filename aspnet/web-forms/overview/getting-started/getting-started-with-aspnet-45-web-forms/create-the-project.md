...
此默认 Web 应用程序中有三个主要页面：*Default.aspx* （主文件夹）， *About.aspx*，和*Contact.aspx*。 每个这些页可从顶部导航栏访问。 也有两个帐户文件夹中包含的其他页、 Register.aspx 页面和 Login.aspx 页面。 这两页，可使用 ASP.NET 成员资格功能来创建、 存储和验证用户凭据。
