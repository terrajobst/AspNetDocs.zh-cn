...
## <a name="adding-order-tracking"></a><span data-ttu-id="bdb0a-127">添加订单跟踪</span><span class="sxs-lookup"><span data-stu-id="bdb0a-127">Adding Order Tracking</span></span>
...
10. <span data-ttu-id="bdb0a-201">单击 **"是"** 将证书安装到本地主机。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-201">Click **Yes** to install the certificate to your localhost.</span></span>   
...
9. <span data-ttu-id="bdb0a-236">将**授权的重定向 URI**设置为以下内容：</span><span class="sxs-lookup"><span data-stu-id="bdb0a-236">Set the **Authorized Redirect URI** to the following:</span></span> 
   ...
   <span data-ttu-id="bdb0a-237">此值是 ASP.NET OAuth 用户与 Google OAuth 服务器通信时使用的 URI。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-237">This value is the URI that ASP.NET OAuth users to communicate with the google OAuth server.</span></span> <span data-ttu-id="bdb0a-238">请记住前面使用的 SSL URL （`https://localhost:44300/`，除非你已创建其他 SSL 项目）。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-238">Remember the SSL URL you used above (    `https://localhost:44300/` unless you've created other SSL projects).</span></span>
...
### <a name="add-paypal-class-and-api-credentials"></a><span data-ttu-id="bdb0a-329">添加 PayPal 类和 API 凭据</span><span class="sxs-lookup"><span data-stu-id="bdb0a-329">Add PayPal Class and API Credentials</span></span>
...
- <span data-ttu-id="bdb0a-347">`SetExpressCheckout` 函数</span><span class="sxs-lookup"><span data-stu-id="bdb0a-347">`SetExpressCheckout` function</span></span>
...
### <a name="review-order-details"></a><span data-ttu-id="bdb0a-394">查看订单详细信息</span><span class="sxs-lookup"><span data-stu-id="bdb0a-394">Review Order Details</span></span>
...
### <a name="complete-purchase"></a><span data-ttu-id="bdb0a-411">完成购买</span><span class="sxs-lookup"><span data-stu-id="bdb0a-411">Complete Purchase</span></span>
...
<span data-ttu-id="bdb0a-417">加载*CheckoutComplete*页面时，将调用 `DoCheckoutPayment` 方法。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-417">When the *CheckoutComplete.aspx* page is loaded, the `DoCheckoutPayment` method is called.</span></span> <span data-ttu-id="bdb0a-418">如前文所述，`DoCheckoutPayment` 方法从 PayPal 测试环境中完成购买。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-418">As mentioned earlier, the `DoCheckoutPayment` method completes the purchase from the PayPal testing environment.</span></span> <span data-ttu-id="bdb0a-419">在 PayPal 完成订单购买后， *CheckoutComplete*页面会显示 `ID` 到买方的付款交易。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-419">Once PayPal has completed the purchase of the order, the *CheckoutComplete.aspx* page displays a payment transaction `ID` to the purchaser.</span></span>

### <a name="handle-cancel-purchase"></a><span data-ttu-id="bdb0a-420">处理取消购买</span><span class="sxs-lookup"><span data-stu-id="bdb0a-420">Handle Cancel Purchase</span></span>

<span data-ttu-id="bdb0a-421">如果用户决定取消购买，则会将其定向到 " *CheckoutCancel* " 页，在该页面中，他们将看到已取消的订单。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-421">If the user decides to cancel the purchase, they will be directed to the *CheckoutCancel.aspx* page where they will see that their order has been cancelled.</span></span>

1. <span data-ttu-id="bdb0a-422">在*签出*文件夹中打开名为*CheckoutCancel*的页。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-422">Open the page named *CheckoutCancel.aspx* in the *Checkout* folder.</span></span>
2. <span data-ttu-id="bdb0a-423">将现有标记替换为以下内容：</span><span class="sxs-lookup"><span data-stu-id="bdb0a-423">Replace the existing markup with the following:</span></span>   

    [!code-aspx[Main](checkout-and-payment-with-paypal/samples/sample24.aspx)]

### <a name="handle-purchase-errors"></a><span data-ttu-id="bdb0a-424">处理采购错误</span><span class="sxs-lookup"><span data-stu-id="bdb0a-424">Handle Purchase Errors</span></span>

<span data-ttu-id="bdb0a-425">在购买过程中的错误将由*CheckoutError*页面处理。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-425">Errors during the purchase process will be handled by the *CheckoutError.aspx* page.</span></span> <span data-ttu-id="bdb0a-426">如果发生错误，则*CheckoutStart*页的代码隐藏、 *CheckoutReview*页和*CheckoutComplete*页将每个都重定向到*CheckoutError*页。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-426">The code-behind of the *CheckoutStart.aspx* page, the *CheckoutReview.aspx* page, and the *CheckoutComplete.aspx* page will each redirect to the *CheckoutError.aspx* page if an error occurs.</span></span>

1. <span data-ttu-id="bdb0a-427">在*签出*文件夹中打开名为*CheckoutError*的页。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-427">Open the page named *CheckoutError.aspx* in the *Checkout* folder.</span></span>
2. <span data-ttu-id="bdb0a-428">将现有标记替换为以下内容：</span><span class="sxs-lookup"><span data-stu-id="bdb0a-428">Replace the existing markup with the following:</span></span>   

    [!code-aspx[Main](checkout-and-payment-with-paypal/samples/sample25.aspx)]

<span data-ttu-id="bdb0a-429">在签出过程中出现错误时，会显示*CheckoutError*页，其中包含错误详细信息。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-429">The *CheckoutError.aspx* page is displayed with the error details when an error occurs during the checkout process.</span></span>

## <a name="running-the-application"></a><span data-ttu-id="bdb0a-430">运行应用程序</span><span class="sxs-lookup"><span data-stu-id="bdb0a-430">Running the Application</span></span>

<span data-ttu-id="bdb0a-431">运行应用程序以了解如何购买产品。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-431">Run the application to see how to purchase products.</span></span> <span data-ttu-id="bdb0a-432">请注意，你将在 PayPal 测试环境中运行。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-432">Note that you will be running in the PayPal testing environment.</span></span> <span data-ttu-id="bdb0a-433">不会交换实际资金。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-433">No actual money is being exchanged.</span></span>

1. <span data-ttu-id="bdb0a-434">请确保所有文件都保存在 Visual Studio 中。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-434">Make sure all your files are saved in Visual Studio.</span></span>
2. <span data-ttu-id="bdb0a-435">打开 Web 浏览器并导航到[https://developer.paypal.com](https://developer.paypal.com/)。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-435">Open a Web browser and navigate to [https://developer.paypal.com](https://developer.paypal.com/).</span></span>
3. <span data-ttu-id="bdb0a-436">登录到你之前在本教程中创建的 PayPal 开发人员帐户。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-436">Login with your PayPal developer account that you created earlier in this tutorial.</span></span>  
   <span data-ttu-id="bdb0a-437">对于 PayPal 的开发人员沙箱，需要在[https://developer.paypal.com](https://developer.paypal.com/)登录，以测试快速结帐。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-437">For PayPal's developer sandbox, you need to be logged in at [https://developer.paypal.com](https://developer.paypal.com/) to test express checkout.</span></span> <span data-ttu-id="bdb0a-438">这仅适用于 PayPal 的沙箱测试，而不适用于 PayPal 的实时环境。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-438">This only applies to PayPal's sandbox testing, not to PayPal's live environment.</span></span>
4. <span data-ttu-id="bdb0a-439">在 Visual Studio 中，按**F5**运行 "Wingtip 玩具" 示例应用程序。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-439">In Visual Studio, press **F5** to run the Wingtip Toys sample application.</span></span>  
   <span data-ttu-id="bdb0a-440">重新生成数据库后，浏览器将打开并显示*default.aspx*页。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-440">After the database rebuilds, the browser will open and show the *Default.aspx* page.</span></span>
5. <span data-ttu-id="bdb0a-441">选择产品类别（如 "汽车"），然后单击每个产品旁边的 "**添加到购物车**"，将三个不同的产品添加到购物车。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-441">Add three different products to the shopping cart by selecting the product category, such as "Cars" and then clicking **Add to Cart** next to each product.</span></span>  
   <span data-ttu-id="bdb0a-442">购物车将显示您选择的产品。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-442">The shopping cart will display the product you have selected.</span></span>
6. <span data-ttu-id="bdb0a-443">单击**PayPal**按钮以结帐。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-443">Click the **PayPal** button to checkout.</span></span> 

    ![通过 PayPal 购物车结帐和付款](checkout-and-payment-with-paypal/_static/image20.png)

   <span data-ttu-id="bdb0a-445">签出将要求你拥有 Wingtip 玩具示例应用程序的用户帐户。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-445">Checking out will require that you have a user account for the Wingtip Toys sample application.</span></span>
7. <span data-ttu-id="bdb0a-446">单击页面右侧的**Google**链接，使用现有的 gmail.com 电子邮件帐户登录。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-446">Click the **Google** link on the right of the page to log in with an existing gmail.com email account.</span></span>  
   <span data-ttu-id="bdb0a-447">如果没有 gmail.com 帐户，可以在[www.gmail.com](https://www.gmail.com/)上创建一个用于测试目的。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-447">If you do not have a gmail.com account, you can create one for testing purposes at [www.gmail.com](https://www.gmail.com/).</span></span> <span data-ttu-id="bdb0a-448">还可以通过单击 "注册" 来使用标准本地帐户。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-448">You can also use a standard local account by clicking "Register".</span></span> 

    ![签入并支付 PayPal-登录](checkout-and-payment-with-paypal/_static/image21.png)
8. <span data-ttu-id="bdb0a-450">用 gmail 帐户和密码登录。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-450">Sign in with your gmail account and password.</span></span> 

    ![通过 PayPal 登录进行结帐和付款](checkout-and-payment-with-paypal/_static/image22.png)
9. <span data-ttu-id="bdb0a-452">单击 "**登录**" 按钮，将 gmail 帐户注册到 Wingtip 玩具示例应用程序用户名。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-452">Click the **Log in** button to register your gmail account with your Wingtip Toys sample application user name.</span></span> 

    ![签出并支付 PayPal-注册帐户](checkout-and-payment-with-paypal/_static/image23.png)
10. <span data-ttu-id="bdb0a-454">在 PayPal 测试网站上，添加之前在本教程中创建的**买家**电子邮件地址和密码，并单击 "**登录**" 按钮。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-454">On the PayPal test site, add your **buyer** email address and password that you created earlier in this tutorial, then click the **Log In** button.</span></span> 

    ![通过 PayPal-PayPal 登录进行结帐和付款](checkout-and-payment-with-paypal/_static/image24.png)
11. <span data-ttu-id="bdb0a-456">同意 PayPal 政策，并单击 "**同意并继续**" 按钮。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-456">Agree to the PayPal policy and click the **Agree and Continue** button.</span></span>  
    <span data-ttu-id="bdb0a-457">请注意，仅当首次使用此 PayPal 帐户时，才会显示此页。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-457">Note that this page is only displayed the first time you use this PayPal account.</span></span> <span data-ttu-id="bdb0a-458">再次请注意，这是一个测试帐户，不会交换真正的资金。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-458">Again note that this is a test account, no real money is exchanged.</span></span> 

    ![结帐和支付 PayPal-PayPal 政策](checkout-and-payment-with-paypal/_static/image25.png)
12. <span data-ttu-id="bdb0a-460">查看 PayPal 测试环境检查页上的订单信息，然后单击 "**继续**"。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-460">Review the order information on the PayPal testing environment review page and click **Continue**.</span></span> 

    ![结帐和支付 PayPal-查看信息](checkout-and-payment-with-paypal/_static/image26.png)
13. <span data-ttu-id="bdb0a-462">在 " *CheckoutReview* " 页上，验证订单量并查看生成的寄送地址。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-462">On the *CheckoutReview.aspx* page, verify the order amount and view the generated shipping address.</span></span> <span data-ttu-id="bdb0a-463">然后单击 "**完成顺序**" 按钮。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-463">Then, click the **Complete Order** button.</span></span> 

    ![通过 PayPal 订购审核结帐和付款](checkout-and-payment-with-paypal/_static/image27.png)
14. <span data-ttu-id="bdb0a-465">将显示**CheckoutComplete**页，其中包含付款事务 ID。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-465">The **CheckoutComplete.aspx** page is displayed with a payment transaction ID.</span></span> 

    ![通过 PayPal 结帐和付款-结帐完成](checkout-and-payment-with-paypal/_static/image28.png)

<a id="ReviewDBWebForms"></a>
## <a name="reviewing-the-database"></a><span data-ttu-id="bdb0a-467">查看数据库</span><span class="sxs-lookup"><span data-stu-id="bdb0a-467">Reviewing the Database</span></span>

<span data-ttu-id="bdb0a-468">在运行应用程序后，通过查看 Wingtip 玩具示例应用程序数据库中的更新数据，可以看到该应用程序已成功记录产品的购买情况。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-468">By reviewing the updated data in the Wingtip Toys sample application database after running the application, you can see that the application successfully recorded the purchase of the products.</span></span>

<span data-ttu-id="bdb0a-469">您可以使用 "**数据库资源管理器**" 窗口（Visual Studio 中的 "**服务器资源管理器**" 窗口）检查*Wingtiptoys*数据库文件中包含的数据，就像您在本教程系列中之前所做的那样。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-469">You can inspect the data contained in the *Wingtiptoys.mdf* database file by using the **Database Explorer** window (**Server Explorer** window in Visual Studio) as you did earlier in this tutorial series.</span></span>

1. <span data-ttu-id="bdb0a-470">如果浏览器窗口仍处于打开状态，请将其关闭。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-470">Close the browser window if it is still open.</span></span>
2. <span data-ttu-id="bdb0a-471">在 Visual Studio 中，选择 "**解决方案资源管理器**顶部的"**显示所有文件**"图标，以允许你将**应用展开\_Data**文件夹。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-471">In Visual Studio, select the **Show All Files** icon at the top of **Solution Explorer** to allow you to expand the **App\_Data** folder.</span></span>
3. <span data-ttu-id="bdb0a-472">展开**应用\_Data**文件夹。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-472">Expand the **App\_Data** folder.</span></span>  
 <span data-ttu-id="bdb0a-473">可能需要选择文件夹的 "**显示所有文件**" 图标。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-473">You may need to select the **Show All Files** icon for the folder.</span></span>
4. <span data-ttu-id="bdb0a-474">右键单击*Wingtiptoys*数据库文件，然后选择 "**打开**"。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-474">Right-click the *Wingtiptoys.mdf* database file and select **Open**.</span></span>  
    <span data-ttu-id="bdb0a-475">将显示**服务器资源管理器**。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-475">**Server Explorer** is displayed.</span></span>
5. <span data-ttu-id="bdb0a-476">展开 "**表**" 文件夹。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-476">Expand the **Tables** folder.</span></span>
6. <span data-ttu-id="bdb0a-477">右键单击**Orders**表，然后选择 "**显示表数据**"。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-477">Right-click the **Orders**table and select **Show Table Data**.</span></span>  
 <span data-ttu-id="bdb0a-478">显示 "**订单**" 表。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-478">The **Orders** table is displayed.</span></span>
7. <span data-ttu-id="bdb0a-479">查看 " **PaymentTransactionID** " 列以确认事务是否成功。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-479">Review the **PaymentTransactionID** column to confirm successful transactions.</span></span> 

    ![签出和支付 PayPal-查看数据库](checkout-and-payment-with-paypal/_static/image29.png)
8. <span data-ttu-id="bdb0a-481">关闭 "**订单**表" 窗口。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-481">Close the **Orders** table window.</span></span>
9. <span data-ttu-id="bdb0a-482">在服务器资源管理器中，右键单击**OrderDetails**表并选择 "**显示表数据**"。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-482">In the Server Explorer, right-click the **OrderDetails** table and select **Show Table Data**.</span></span>
10. <span data-ttu-id="bdb0a-483">查看**OrderDetails**表中的 `OrderId` 和 `Username` 值。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-483">Review the `OrderId` and `Username` values in the **OrderDetails** table.</span></span> <span data-ttu-id="bdb0a-484">请注意，这些值匹配**Orders**表中包含的 `OrderId` 和 `Username` 值。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-484">Note that these values match the `OrderId` and `Username` values included in the **Orders** table.</span></span>
11. <span data-ttu-id="bdb0a-485">关闭 " **OrderDetails**表" 窗口。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-485">Close the **OrderDetails** table window.</span></span>
12. <span data-ttu-id="bdb0a-486">右键单击 "Wingtip 玩具" 数据库文件（*Wingtiptoys*），然后选择 "**关闭连接**"。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-486">Right-click the Wingtip Toys database file (*Wingtiptoys.mdf*) and select **Close Connection**.</span></span>
13. <span data-ttu-id="bdb0a-487">如果看不到 "**解决方案资源管理器**" 窗口，请单击 "**服务器资源管理器**" 窗口底部的 "**解决方案资源管理器**以再次显示**解决方案资源管理器**。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-487">If you do not see the **Solution Explorer** window, click **Solution Explorer** at the bottom of the **Server Explorer** window to show the **Solution Explorer** again.</span></span>

## <a name="summary"></a><span data-ttu-id="bdb0a-488">总结</span><span class="sxs-lookup"><span data-stu-id="bdb0a-488">Summary</span></span>

<span data-ttu-id="bdb0a-489">在本教程中，您添加了订单和订单详细信息架构以跟踪产品的购买情况。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-489">In this tutorial you added order and order detail schemas to track the purchase of products.</span></span> <span data-ttu-id="bdb0a-490">还将 PayPal 功能集成到了 Wingtip 玩具示例应用程序中。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-490">You also integrated PayPal functionality into the Wingtip Toys sample application.</span></span>

## <a name="additional-resources"></a><span data-ttu-id="bdb0a-491">其他资源</span><span class="sxs-lookup"><span data-stu-id="bdb0a-491">Additional Resources</span></span>

<span data-ttu-id="bdb0a-492">[ASP.NET 配置概述](https://msdn.microsoft.com/library/ms178683(v=vs.100).aspx)</span><span class="sxs-lookup"><span data-stu-id="bdb0a-492">[ASP.NET Configuration Overview](https://msdn.microsoft.com/library/ms178683(v=vs.100).aspx)</span></span>  
[<span data-ttu-id="bdb0a-493">使用成员资格、OAuth 和 SQL 数据库将安全的 ASP.NET Web 窗体应用程序部署到 Azure App Service</span><span class="sxs-lookup"><span data-stu-id="bdb0a-493">Deploy a Secure ASP.NET Web Forms App with Membership, OAuth, and SQL Database to Azure App Service</span></span>](https://azure.microsoft.com/documentation/articles/web-sites-dotnet-deploy-aspnet-webforms-app-membership-oauth-sql-database/)  
[<span data-ttu-id="bdb0a-494">Microsoft Azure 免费试用版</span><span class="sxs-lookup"><span data-stu-id="bdb0a-494">Microsoft Azure - Free Trial</span></span>](https://azure.microsoft.com/pricing/free-trial/)

## <a name="disclaimer"></a><span data-ttu-id="bdb0a-495">免责声明</span><span class="sxs-lookup"><span data-stu-id="bdb0a-495">Disclaimer</span></span>

<span data-ttu-id="bdb0a-496">本教程包含示例代码。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-496">This tutorial contains sample code.</span></span> <span data-ttu-id="bdb0a-497">此类示例代码 "按原样" 提供，不提供任何形式的保证。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-497">Such sample code is provided "as is" without warranty of any kind.</span></span> <span data-ttu-id="bdb0a-498">因此，Microsoft 不保证代码的准确性、完整性或质量。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-498">Accordingly, Microsoft does not guarantee the accuracy, integrity, or quality of the sample code.</span></span> <span data-ttu-id="bdb0a-499">您同意使用此示例代码的风险由您自己承担。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-499">You agree to use the sample code at your own risk.</span></span> <span data-ttu-id="bdb0a-500">在任何情况下，Microsoft 都不会对任何示例代码、内容（包括但不限于）任何代码、内容或任何类型的任何错误或遗漏的任何错误或遗漏提供任何错误或遗漏，因为使用任何示例代码。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-500">Under no circumstances will Microsoft be liable to you in any way for any sample code, content, including but not limited to, any errors or omissions in any sample code, content, or any loss or damage of any kind incurred as a result of the use of any sample code.</span></span> <span data-ttu-id="bdb0a-501">你将被特此通知并特此同意赔偿，无论是不受您发布的材料的 occasioned，还是从您发布的材料中产生的所有损失、损失、损失、伤害或损失的任何种类（包括但不限于），传输、使用或依赖于其中所含的视图（但不限于）。</span><span class="sxs-lookup"><span data-stu-id="bdb0a-501">You are hereby notified and do hereby agree to indemnify, save and hold Microsoft harmless from and against any and all loss, claims of loss, injury or damage of any kind including, without limitation, those occasioned by or arising from material that you post, transmit, use or rely on including, but not limited to, the views expressed therein.</span></span>

> [!div class="step-by-step"]
> <span data-ttu-id="bdb0a-502">[上一页](shopping-cart.md)
> [下一页](membership-and-administration.md)</span><span class="sxs-lookup"><span data-stu-id="bdb0a-502">[Previous](shopping-cart.md)
