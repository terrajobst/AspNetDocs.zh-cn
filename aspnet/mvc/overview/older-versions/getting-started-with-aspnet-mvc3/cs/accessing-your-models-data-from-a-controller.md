...
ASP.NET MVC 还提供了能够传递强类型化数据或视图模板的对象。 此强类型化方法允许更好地进行编译时检查的代码和更丰富的 IntelliSense，Visual Web Developer 编辑器中。 我们将使用此方法与`MoviesController`类和*Index.cshtml*视图模板。
...
通过包括`@model`语句在视图模板文件的顶部，可以指定视图期望的对象的类型。 创建电影控制器时，Visual Web Developer 中自动包括以下`@model`顶部的语句*Index.cshtml*文件：
