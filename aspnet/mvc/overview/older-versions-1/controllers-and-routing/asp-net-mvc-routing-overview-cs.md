...
在列表2中，HomeController 类包含一个名为 Index （）的方法，该方法接受名为 Id 的单一参数。URL/Home 导致使用空字符串作为 Id 参数的值调用 Index （）方法。

由于 MVC 框架调用控制器操作的方式，URL/Home 还与列表3中 HomeController 类的 Index （）方法匹配。

**列出 HomeController.cs （没有参数的索引操作）**
...
**列表 4-HomeController.cs （索引操作，参数可以为 null）**

[!code-csharp[Main](asp-net-mvc-routing-overview-cs/samples/sample4.cs)]

在列表4中，Index （）方法有一个整数参数。 由于参数是可以为 null 的参数（可以具有值 Null），因此可以在不引发错误的情况下调用索引（）。

最后，在使用 URL/Home 的列表5中调用 Index （）方法将导致异常，因为 Id 参数*不*是可以为 null 的参数。 如果尝试调用 Index （）方法，则会获得图1中显示的错误。

**列出 HomeController.cs （带有 Id 参数的索引操作）**
...
**图 01**：调用需要参数值的控制器操作（[单击以查看完全大小的图像](asp-net-mvc-routing-overview-cs/_static/image2.png)）
...
> [下一部分](understanding-action-filters-cs.md)
