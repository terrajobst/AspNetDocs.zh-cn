...
* 受保护负载的其余部分是[特定于此密钥中的加密程序](xref:security/data-protection/implementation/subkeyderivation#data-protection-implementation-subkey-derivation)。 在下面的示例与该键表示 AES-256-CBC + HMACSHA256 加密程序和有效负载被进一步细分，如下所示: * 的 128 位密钥修饰符。 * 一个 128 位初始化向量。 * AES-256-CBC 输出 48 个字节。 * 一个 HMACSHA256 身份验证标记。
