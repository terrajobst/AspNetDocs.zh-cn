...
## <a name="core"></a>核心
  * 添加绑定用于使用按钮类型输入。 关闭 #1891
  * 支持 jquery3。 关闭 #1866
  * 将 jQuery 别名“expr[":"]”更改为“expr.pseudos”

## <a name="localisation"></a>本地化
  * 添加乌尔都语翻译。 关闭 #1873。

## <a name="localization"></a>本地化
  * 修复了 az 翻译错误的文件扩展名。 关闭 #1890。
  * 在 pt-BR 中添加了缺少的翻译（关闭 #1897）
  * 修复了阿拉伯语语言文件中的拼写错误。
...
## <a name="additional"></a>其他
  * 修复了多个 mime 类型验证
  * IBAN 需要至少 5 个字符（关闭 # 1797，修复 # 1674）
  * 更正了 notEqualTo jQuery 引用

## <a name="core"></a>核心
  * 添加了 #1805 的失败测试
  * 修复了 3 个和更多个字段的组验证
  * 修复了 #1644 和 #1657 中引入的回归（关闭 #1800）
  * 将步骤验证更新为正确处理浮点
  * 修复了在非表单元素上调用 $.fn.rules() 的错误
  * 在验证程序作用域中调用 `errorPlacement`
  * 修复了表单中的 contenteditable 元素的问题，其中单个输入验证的事件会导致异常

## <a name="demo"></a>演示
  * 向 Bootstrap 添加了连接，向 index.html 添加了语义 UI 演示
  * 使用 `.on()` 替代 `.validateDelegate()`
...
## <a name="core"></a>核心
  * `resetForm` 还应从元素中删除 `valid` 类。
  * 如果在使用远程规则时已突出显示字段，则取消突出显示。
  * 在 `equalTo` 规则中仅绑定 `blur` 事件一次
  * 修复了在空 jquery 集上调用.rules() 时出现的错误。
  * 修复了输入组处理错误消息的问题。
  * 修复了在使用 `groups` 设置时的 `showLabel` 类型错误
  * 添加了将方法名称传递给远程位置的方法
  * 当下一个字段已经填写时，验证无法触发（修复 #1508）
  * 所需的规则优先于数量和数字规则
  * 错误隐藏但输入错误类未删除
  * 远程验证使用不正确的错误消息
  * 修复了远程验证的字段突出显示问题。
  * 修复了用于多个选择元素的 `:filled` 选择器。
  * 向 jQuery.validator.methods 添加了文档引用
  * 将消息处理从 `formatAndAdd` 移动到 `defaultMessage`
  * 错误列表只能包含预期错误
  * 提取文件名，不包括 "C:\fakepath\"
  * HTML5 步骤特性支持。 修复 #1295
  * 在未完成的请求中添加了对“待处理”类的支持
  * 添加了规范化程序 (#1602)
  * 拆分 `creditcard` 方法
  * 转义在正则表达式中使用的 errorID，而不是构建 aria-describedby
  * 转义名称中的单引号，避免引发 Sizzle 错误
  * 不使用验证字段的值来跳过 api 调用，而是使用请求的序列化数据对象
  * 添加了对 contentEditable 标记的支持

## <a name="additional"></a>其他
  * BIC：在第二个位置允许存在数字 1-9
  * 接受方法正则表达式应正确转义。
  * BIC 检查不区分大小写
  * 更正 postalCodeCA 以排除无效组合
  * 使 postalCodeCA 方法更宽松

## <a name="localization"></a>本地化
  * 添加了马其顿语本地化。
  * 添加了波兰语中缺少的模式消息 (adamwojtkiewicz)
  * 修复了最小/最大消息的波斯语翻译。
  * 更新了 messages_sk.js
  * 更新马来语翻译
  * 包含了来自其他方法的消息
  * 提升了 pt_BR 翻译质量，并修复了“cifES”键上的拼写错误。
...
* 不要使用 form.elements，因为 jQuery 1.6 中不再为其提供支持。 而且它会发生错误 (IE6-8: form.elements === form)。
...
* 已修复信用卡验证以仅接受数字和短划线（asdf 是无效的信用卡号）
...
* 完全删除元素 ID 的依赖关系，尽管仍然使用它们（存在时）将错误标签链接到输入。 通过使用具有 {name, message, element} 的数组来实现，而不是使用具有 id:message 对（用于内部 errorList）的对象。
...
* 已简化 API 以支持不同的事件。 可以禁用默认值 submit。 如果已指定事件，将其应用于每个元素（而不是整个窗体）。 将 keyup-validation 与 submit-validation 组合在一起，现在非常容易设置
