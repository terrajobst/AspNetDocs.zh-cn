...
有关详细信息，请参阅 <xref:razor-components/hosting-models#server-side-hosting-model>。

Blazor 是 Razor 组件的实验性客户端托管模型。 Blazor 在使用开放 Web 标准的浏览器中的 .NET 上运行，没有插件，也没有代码转译。 有关详细信息，请参阅 <xref:razor-components/hosting-models#client-side-hosting-model>。
